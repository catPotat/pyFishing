...
'''Boomerang, mirror, rotate option by Long Nguyen
Wait no, rotation does not work :( Anyone who helps figuring it out is much appreciated'''
...
class GIFImage(object):
    def __init__(self, filename):
        ...
        self.startpoint = 0
        self.reversed = False
        self.doboomerang = False
        self.nextframe = 1

    def get_rect(self):
        return pygame.rect.Rect((0,0), self.image.size)

    def get_frames(self, mirrored=False, horizontal=True, rotated=0):
        image = self.image
        ...
        try:
            while 1:
                ...
                if mirrored:
                    if horizontal:
                        pi= pygame.transform.flip(pi, True, False)
                    else:
                        pi= pygame.transform.flip(pi, False, True)
                if rotated:
                    pi = pygame.transform.rotate(pi, rotated)
    ...
    def render(self, screen, pos):
        if self.running:
            if time.time() - self.ptime > self.frames[self.cur][1]:
                if self.doboomerang:
                    self.cur += self.nextframe
                    if self.cur <= self.startpoint or self.cur >= self.breakpoint:
                        self.nextframe *= -1
                elif self.reversed:
                    self.cur -= 1
    ...
    def transform(self, mirrored=False, horizontal=True, rotated=0):
        self.frames[:] = []
        self.get_frames(mirrored, horizontal, rotated)
    ...
    def reset(self):
        ...
        self.reversed = False
    def boomerang(self):
        self.doboomerang = not self.doboomerang
